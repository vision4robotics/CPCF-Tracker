...
%             scale_change_factor = scaleFactors(sind);
    ...
    for k = 1: 1
           ...
           %Adjsut the magnitude of the z_label
            magnitude = mag_min+(mag_max-mag_min)*(psrm/50);
            for i = 1:num_feature_blocks
                sz = filter_sz_cell{i};
                output_sigma = sqrt(prod(floor(base_target_sz/feature_cell_sz(i)))) * rc_sigma_factor;
                rg           = circshift(-floor((sz(1)-1)/2):ceil((sz(1)-1)/2), [0 -floor((sz(1)-1)/2)]);
                cg           = circshift(-floor((sz(2)-1)/2):ceil((sz(2)-1)/2), [0 -floor((sz(2)-1)/2)]);
                [rs, cs]     = ndgrid(rg,cg);
            ...
            A = mu ./ (all1 + admm_gamma*(conj(M_prev).*M_prev));          
            B = S_xx + T * A;
            C = 1./A;
            S_lx = sum(conj(model_xf) .* l_f, 3);
            S_hx = sum(conj(model_xf) .* h_f, 3);
            model_xmf = bsxfun(@times, M_prev,model_xf);
...
[~, results] = get_sequence_results(seq, all_psr, all_Rmax, all_psrm);
